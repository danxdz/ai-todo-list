...
export function createSfx() {
  ...
  return {
    ...
    /** Distinct chemistry fusion sting (used by atoms molecule crafting). */
    playMolecule(size = 3) {
      if (muted) return;
      resume();
      const w = Math.min(2.6, Math.max(1, size / 4));
      beep(440, 0.1, 0, 'triangle', 0.44);
      beep(660, 0.14, 0.06, 'sine', 0.4);
      beep(990, 0.2, 0.14, 'sine', Math.min(0.62, 0.35 + w * 0.08));
      beep(1320, 0.24, 0.24, 'sine', Math.min(0.58, 0.31 + w * 0.06));
    },
  };
}
